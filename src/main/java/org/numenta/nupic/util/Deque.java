 ...
 * end, thus always maintaining a size &lt;= initial size.具有受限容量的双端队列实现。操作可以在两端进行，当容量达到时，对任意一端添加的值将会导致另外一端的一个值被删除，因此总是保持一个小于等于初始化尺寸的大小
 * 
...
public class Deque<E> implements Iterable<E>, Serializable {
	...
	/**
	 * Constructs a new {@code Deque} with the specified capacity. 用指定的容量构建一个双端数列
	 * @param capacity
	 */
	...
	public boolean append(E t) {
		boolean ret = currentSize == capacity;//首先先判断一下容量是否达到最大
		if(ret) {
			backingList.removeFirst();
			backingList.addLast(t);
		}else{
			backingList.addLast(t);//加入队列
			currentSize++;//记录当前的赤诚
		}
		return ret;
	}
