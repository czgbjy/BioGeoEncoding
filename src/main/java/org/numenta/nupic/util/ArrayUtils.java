...
public class ArrayUtils {
   ...
   /**
     ...
     * 返回一个数组，数组的大小和输入数组是一样的，但是输入数组中的值都被转换为int型
     ...
     * 执行Python的求模运算，和求余运算不同，这里的实现代码有问题，mark
     * @param a
     * @param b
     * @return
     */
    public static int modulo_(int a, int b) {
     ...
     * 把输入的两个数组中相对应的每个元素中的较大值挑选出来组成一个新的数组
     * @param arr1
     * @param arr2
     * @return
     */
     ...
     * 返回一个数组，数组中的值是除数数组中的值和被除数数组中值的商
     * @param dividend 被除数
     * @param divisor 除数
     * @param dividend adjustment 被除数的校正值
     * @param divisor  adjustment 除数的校正值
     *
     * @return
    ...
    public static double[] divide(double[] dividend, double[] divisor,
        ...
        double[] quotient = new double[dividend.length];//商(除法所得的结果)
        double denom = 1;
     ...
     * @param factor 乘数因子
     * @param multiplicandAdjustment 乘数调整因子
     * @param factorAdjustment 乘数因子调整因子
     *
     * @return
     * @throws IllegalArgumentException if the two argument arrays are not the same length
     */
    public static double[] multiply(
        ...
        double[] product = new double[multiplicand.length];//成绩数组
     ...
     * 返回一个数组，从lowerBounds开始（包括），到upperBounds（不包括）
     * @param lowerBounds
     * @param upperBounds
     * @return
     */
     ...
     * to the value "setTo".
     * 指定数据的指定索引处的值设置为指定的值
     * @param values  the values to alter if at the specified indexes. 指定的数组
     * @param indexes the indexes of the values array to alter 指定的索引
     * @param setTo   the value to set at the specified indexes. 要设置的值
     */
     ...
     * to the value "setTo".
     * 把数组中指定的索引处的值设置为指定的值
     * @param values  the values to alter if at the specified indexes. 要被设置值的数组
     * @param indexes the indexes of the values array to alter 要被设置为新值的索引数组
     * @param setTo   the value to set at the specified indexes.要被设置为的值
     */
     ...
     * Returns a random, sorted, and  unique array of the specified sample size of
     * selections from the specified list of choices.
     * 从指定的选择列表中，返回指定样本大小的随机，排序和唯一数组
     * @param sampleSize the number of selections in the returned sample 在返回样本中选择的数量
     * @param choices    the list of choices to select from 去选择的列表
     * @param random     a random number generator 随机数产生器
     * @return a sample of numbers of the specified size  返回指定数量大小的样本
     */
     ...
     *确保指定的数组中的每个元素的值在指定的最小值和最大值之间，如果比指定的最小值或者最大值小或者大，都被被截取
     ...
     * equal to or greater than the min at the specified index and a maximum value less
     * than or equal to the max at the specified index.
     * 也就是让输入的值不得大于给定的最大值，如果大于这个最大值，就取最大值
     * @param values the values to clip 输入值
     * @param max    the minimum value 允许的最大值
     * @param adj    the adjustment amount 允许的最大的值的调整值
     */
     ...
     * Returns the count of values in the specified array that are
     * greater than the specified compare value
     *返回在指定的数组中其值大于指定的比较值的数量
     * @param compare the value to compare to 被比较的值
     * @param array   the values being compared 比较的数组
     *
     * @return the count of values greater 大于被比较的值的数量
     */
    ...
    /**
     * Raises the values at the indexes specified by the amount specified.增加指定数组中指定索引的元素的值，增加的量为amount指定
     * @param amount the amount to raise the values 值增加的量
     * @param values the values to raise 要增加值的数组
     */
     ...
     * value, returning the indexes of the values where the condition evaluates
     * to true.
     * 扫描指定的值并将条件应用于每个值，返回条件计算为true的值的索引。
     * @param values the values to test
     * @param c      the condition used to test each value
     * @return
     */
     ...
     * "x" value, equal to the specified "y". 使得指定数组中小于或者等于x值的元素的值等于y
     * @param array 数组
     * @param x     the comparison 被比较值
     * @param y     the value to set if the comparison fails 如果小于或者等于x,被设定的值
     */
    ...
    /**
     * Sets value to "y" in "targetB" if the value in the same index in "sourceA" is bigger than "x".如果“sourceA”中同一索引中的值大于“x”，则在“targetB”中将值设置为“y”
     ...
     * 返回指定索引的指定的子数组，也就是指定数组的一部分指定元素被拿走构建了一个新的数组
     * @param source
     * @param indexes
     * @return
     */
    ...
    public static void setValue(Object array, int value, int... indexes) {
        if (indexes.length == 1) {
            ((int[])array)[indexes[0]] = value;//设置数组的第indexes[0]个元素的值等于输入值
        } else {
    ...
    public static Object getValue(Object array, int... indexes) {
        Object slice = array;
        for(int i = 0;i < indexes.length;i++) {
            slice = Array.get(slice, indexes[i]);//请看清楚，它获取的不是多个数组，而是指定位置的数组，当索引为两个数字时，它获取的是二维数组中的某个元素的值
        }
    ...
    /**
    * Aggregates all element of multi dimensional array of ints  把int多维数组的所有元素进行求和
    * @param array
    * @return sum of all array elements
    */
    public static int aggregateArray(Object array) {
        int sum = 0;
        if(array instanceof Integer){//如果传入的对象是一个Integer对象
            return (int)array;//直接返回这个对象
        } else if (array instanceof int[]) {//如果传入的是一个int[]
            int[] set = (int[])array;
            for (int element : set) {
                sum += element;//对数组的所有元素进行加和
            }
            return sum;
        } else {
            for (Object agr : (Object[])array) {
